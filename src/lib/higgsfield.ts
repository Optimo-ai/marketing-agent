...
export interface HiggsfieldImageOptions {
  ...
  referenceImageUrl?: string   // URL pública HTTPS accesible por Higgsfield
}
...
export async function generateImage(opts: HiggsfieldImageOptions): Promise<Buffer> {
  ...
  const refUrl = opts.referenceImageUrl
  const payload: any = {
    model,
    prompt: opts.prompt,
  ...
  if (refUrl) {
    payload.medias = [{ value: refUrl, role: 'image' }]
  }

  console.log(`[higgsfield] Submitting image — model: ${model}, aspect: ${opts.aspectRatio ?? '1:1'}${refUrl ? ' (i2i: ' + refUrl.split('/').pop() + ')' : ''}`)
  const submitRes = await fetchWithRetry(`${BASE_URL}/generations`, {
    method: 'POST',
...
export async function generateImageTracked(opts: HiggsfieldImageOptions): Promise<{ buffer: Buffer; jobId: string }> {
  ...
  console.log(`[higgsfield] Submitting image (tracked) — model: ${model}, aspect: ${opts.aspectRatio ?? '1:1'}${refUrl ? ' (i2i: ' + refUrl.split('/').pop() + ')' : ''}`)
  const submitRes = await fetchWithRetry(`${BASE_URL}/generations`, {
    method: 'POST',
    headers: { 'Authorization': `Bearer ${key}`, 'Content-Type': 'application/json' },
    body: JSON.stringify(payload),
